    ...
    render() {
        return ( 
            <div className = "driver-home" >
                ...
                ( <option key = { index } value = { item.username } > { item.id } &nbsp; { item.username } </option>))}
                ...
                ) : null} { /* {console.log("Filtering", (this.state.rideDetails.filter(items => ((items.user === this.state.username && items.status !== "AC") || (items.user === this.state.username && items.status !== "RE"))).length > 0))} */ } { /* {console.log("ride", this.state.rideDetails)} */ }
                 {this.state.rideDetails.filter(items => (items.user === this.state.username && items.status === "RE")
                    )
                    .map((item, index) => ( <div key = { index } className = "driver-on-going" > { /* {console.log("drivername", item.driver)} */ } 
...

            {
                this.state.rideDetails
                    ...
                    .map((item, index) => 
                    ( <div key = { index } className = "driver-on-going" > { /* {console.log("drivername", item.driver)} */ }
                     <h2 >You are on the CAB with { item.driver } 
                     < button onClick = { () => this.endTrip(item.id) } > End Trip </button> 
                     </h2> 
                    </div>
                    ))
            }

            {
                ...
                ( <div className = "driver-ride" >
                    ...
                    <tbody > {this.state.rideDetails.filter(items => items.user === this.state.username).map((item, index) => ( <tr key = { index } > { /* {console.log("table content", item)} */ } <td > { item.driver } </td>
                     <td > { item.ride_created } </td> <td > { item.status } </td> </tr>))
                    }
                    </tbody> </table>
                    </div>
                ) : null
